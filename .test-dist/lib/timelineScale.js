...
exports.getFitPxPerDay = getFitPxPerDay;
exports.getZoomPercent = getZoomPercent;
exports.buildScale = buildScale;
exports.buildDensityScale = buildDensityScale;
...
function clampPxPerDay(pxPerDay) {
    return Math.min(exports.MAX_PX_PER_DAY, Math.max(exports.MIN_PX_PER_DAY, pxPerDay));
}
function getFitPxPerDay(currentPxPerDay, containerWidth, totalWidthPx, labelWidth) {
...
function getZoomPercent(pxPerDay, basePxPerDay) {
    const safeBasePxPerDay = basePxPerDay > 0 ? basePxPerDay : exports.DEFAULT_PX_PER_DAY;
...
function buildDensityScale(events, domainStart, domainEnd, pxPerDay) {
    const domainStartMs = domainStart.getTime();
    const domainEndMs = domainEnd.getTime();
    const totalDays = (domainEndMs - domainStartMs) / MS_PER_DAY;
    const totalWidthPx = Math.max(Math.ceil(totalDays * pxPerDay), 1) + 2 * TIMELINE_EDGE_PX;
    ...
    for (const ev of events) {
        ...
        if (y < startYear || y > endYear)
            continue;
        if (!byCountry.has(ev.country))
            byCountry.set(ev.country, []);
    ...
    const BASE_PX = 4;
    const reqPxPerDay = new Array(numYears).fill(0);
    const hasEvent = new Array(numYears).fill(false);
    for (const ev of events) {
        const y = ev.start_date.getFullYear();
        if (y >= startYear && y <= endYear)
            hasEvent[y - startYear] = true;
    }
    ...
    const cumulativePx = new Array(numYears + 1).fill(0);
    for (let i = 0; i < numYears; i++) {
    ...
    function toPixel(date) {
        ...
        if (y < startYear)
            return TIMELINE_EDGE_PX;
        if (y > endYear)
            return TIMELINE_EDGE_PX + effPlotWidth;
