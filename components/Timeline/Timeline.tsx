...
<TimelineRows events={events} scale={scale} onEventClick={handleEventClick} onTypeFilter={onTypeFilter} />
          {/* Year grid lines — rendered after rows so they appear over row backgrounds */}
          <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 1 }}>
            {yearTicks.map(({ year, x }) => (
              <div
                key={year}
                className="absolute inset-y-0 w-px bg-black/[0.04] dark:bg-white/[0.04]"
                style={{ left: x }}
              />
            ))}
          </div>
        ...
        >
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-[10px] text-content-tertiary font-semibold tracking-wide">
              Desenvolvido por{" "}
              <a
                href="https://www.linkedin.com/in/calegaribr/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-0.5 text-content-secondary font-bold underline hover:text-brand transition-colors"
              >
                Diogo S. P. Calegari
